...
export const MODELING_GUIDANCE = [
    ...
    '',
    'Workflow: call getModelSummary first to see the current state. Create a diagram if needed (STRIDE unless',
    'told otherwise). Add the elements, connect them with flows, group them with trust boundaries, then',
    'enumerate threats. Call validateModel after a batch of changes and again before finishing.',
    '',
    'Readable layout:',
    ...
    '- Space elements generously (about 150-200px apart) so shapes, labels and flows do not overlap or stack.',
    '  Never place two elements at the same or near-identical position.',
    '- Add a trust boundary (kind "box") around each set of components in the same trust zone (e.g. everything',
    '  inside your VPC), separating it from untrusted actors outside.',
